...
function buildClubTeamSummary(save, clubId) {
  ...
  return {
    formation: "4-3-3",
    chemistry: 15 + Math.floor(Math.random() * 10),
...
function createTeamService({ repo, userSaveService }) {
  ...
  function setFormationPlayer(username, position, playerId) {
    ...
    // BUG8 FIX: Normalisasi playerId ke String
    ...
    // FIX v8: Role check lebih fleksibel — GK tetap ketat
    ...
    // Jika posisi sudah terisi player lain → swap
    ...
    // Hapus player dari posisi lain jika ada
    ...
    // Simpan dengan ID asli dari player object
    saveData.lineup[position] = player.id;
    save(username, saveData);
    
    // ✅ FIX: Return formation + team summary
    return {
      formation: getFormation(username),
      team: buildTeamSummary(saveData)
    };
  }
